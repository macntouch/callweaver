...
CALLWEAVER_FILE_VERSION("$HeadURL: https://svn.callweaver.org/callweaver/branches/rel/1.2/apps/app_meetme.c $", "$Revision: 5371 $")
...
zapretry:
    ...
    if (ztc.confmode)
    {
        ...
        if (!retryzap)
        {
            cw_log(LOG_DEBUG, "DAHDI channel is in a conference already, retrying with pseudo\n");
            retryzap = 1;
            goto zapretry;
        }
    }
            ...
            if (c)
            {
                ...
                if ((f->frametype == CW_FRAME_VOICE) && (f->subclass == CW_FORMAT_SLINEAR))
                {
                    ...
                    if (using_pseudo)
                    {
						   ...
						   as fcw as it arrives, and feed it into the conference.
						   The buffering in the pseudo channel will take care of any
						   timing differences, unless they are so drcwic as to lose
...
static struct cw_conference *find_conf(struct cw_channel *chan, char *confno, int make, int dynamic, char *dynamic_pin)
{
    ...
    if (!cnf)
    {
        ...
        {
            /* Check the config */
            cfg = cw_config_load(CONFIG_FILE_NAME);
            if (!cfg)
            {
                cw_log(LOG_WARNING, "No %s file :(\n", CONFIG_FILE_NAME);
                return NULL;
            }
