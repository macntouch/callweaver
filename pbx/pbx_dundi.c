...
CALLWEAVER_FILE_VERSION("$HeadURL: https://svn.callweaver.org/callweaver/branches/rel/1.2/pbx/pbx_dundi.c $", "$Revision: 4723 $")
...
static const char *dundi_lookup_descrip = 
	...
	"answer was found, the variable ${DUNDISTATUS} will be set to FAILED.\n"
	"Otherwise, it will be set to SUCCESS. Note that this will only occur if\n"
	"the global priority jumping option is enabled in extensions.conf. If the\n"
	"'b' option is specified, the internal DUNDi cache will by bypassed.\n";
	...
	if (s > 0) {
		...
		for(x=0;x<10;x++) {
			...
			if (!ioctl(s, SIOCGIFHWADDR, &ifr)) {
				...
				cw_log(LOG_DEBUG, "Seeding global EID '%s' from '%s'\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &global_eid), ifr.ifr_name);
				close(s);
				return;
			}
        }
...
#endif
#endif
	cw_log(LOG_NOTICE, "No ethernet interface found for seeding global EID  You will have to set it manually.\n");
}
...
static void *dundi_lookup_thread(void *data)
{
	...
	cw_log(LOG_DEBUG, "Whee, looking up '%s@%s' for '%s'\n", st->called_number, st->called_context, 
	...
	if (cw_test_flag(st->trans, FLAG_DEAD)) {
		cw_log(LOG_DEBUG, "Our transaction went away!\n");
		st->trans->thread = 0;
...
static void *dundi_precache_thread(void *data)
{
	...
	cw_log(LOG_DEBUG, "Whee, precaching '%s@%s' for '%s'\n", st->called_number, st->called_context, 
		st->eids[0] ? dundi_eid_to_str(eid_str, sizeof(eid_str), st->eids[0]) :  "ourselves");
	memset(&ied, 0, sizeof(ied));
...
static void *dundi_query_thread(void *data)
{
	...
	if (!dundi_eid_cmp(&st->trans->us_eid, &st->reqeid)) {
		/* Ooh, it's us! */
		cw_log(LOG_DEBUG, "Neat, someone look for us!\n");
...
static int dundi_answer_entity(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	if (st) {
		...
		cw_log(LOG_DEBUG, "Answering EID query for '%s@%s'!\n", dundi_eid_to_str(eid_str, sizeof(eid_str), ies->reqeid), ies->called_context);
		pthread_attr_init(&attr);
		pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
		trans->thread = 1;
		if (cw_pthread_create(&lookupthread, &attr, dundi_query_thread, st)) {
			trans->thread = 0;
...
static int cache_save_hint(dundi_eid *eidpeer, struct dundi_request *req, struct dundi_hint *hint, int expiration)
{
	...
	cw_log(LOG_DEBUG, "Caching hint at '%s'\n", key1);
...
static int dundi_prop_precache(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	for (x=0;x<ies->anscount;x++) {
		if (trans->parent->respcount < trans->parent->maxcount) {
			...
			cw_log(LOG_NOTICE, "Dropping excessive answers in precache for %s@%s\n",
	...
	if (st) {
		...
		st->nummaps = mapcount;
		cw_log(LOG_DEBUG, "Forwarding precache for '%s@%s'!\n", ies->called_number, ies->called_context);
		pthread_attr_init(&attr);
		pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
		trans->thread = 1;
		if (cw_pthread_create(&lookupthread, &attr, dundi_precache_thread, st)) {
			trans->thread = 0;
			cw_log(LOG_WARNING, "Unable to create thread!\n");
			free(st);
			memset(&ied, 0, sizeof(ied));
...
static int dundi_answer_query(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	if (st) {
		...
		st->nummaps = mapcount;
		cw_log(LOG_DEBUG, "Answering query for '%s@%s'!\n", ies->called_number, ies->called_context);
		pthread_attr_init(&attr);
		pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
		trans->thread = 1;
		if (cw_pthread_create(&lookupthread, &attr, dundi_lookup_thread, st)) {
			trans->thread = 0;
			cw_log(LOG_WARNING, "Unable to create thread!\n");
			free(st);
			memset(&ied, 0, sizeof(ied));
...
static int cache_lookup_internal(time_t now, struct dundi_request *req, char *key, char *eid_str_full, int *lowexpiration)
{
	...
	if (!cw_db_get("dundi/cache", key, data, sizeof(data))) {
		...
		if (sscanf(ptr, "%ld,%n", &timeout, &length) == 1) {
			...
			if (expiration > 0) {
				cw_log(LOG_DEBUG, "Found cache expiring in %d seconds!\n", (int)(timeout - now));
				ptr += length;
						...
						*term = '\0';
						...
						if (src) {
							*src = '\0';
							...
							src = "";
						cw_log(LOG_DEBUG, "Found cached answer '%s/%s' originally from '%s' with flags '%s' on behalf of '%s'\n", 
...
static int do_register_expire(void *data)
{
	...
	cw_log(LOG_DEBUG, "Register expired for '%s'\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
...
static struct dundi_hdr *dundi_decrypt(struct dundi_transaction *trans, unsigned char *dst, int *dstlen, struct dundi_hdr *ohdr, struct dundi_encblock *src, int srclen)
{
	...
	*h = *ohdr;
	...
	if (uncompress(dst + 6, &bytes, decrypt_space, srclen) != Z_OK) {
		cw_log(LOG_DEBUG, "Ouch, uncompress failed :(\n");
		return NULL;
	}
	/* Update length */
	*dstlen = bytes + 6;
...
static int dundi_encrypt(struct dundi_transaction *trans, struct dundi_packet *pack)
{
	...
	if (res != Z_OK) {
		cw_log(LOG_DEBUG, "Ouch, compression failed!\n");
		return -1;
	}
	memset(&ied, 0, sizeof(ied));
	/* Say who we are */
	...
	if ((ied.pos + bytes) >= sizeof(ied.buf)) {
		cw_log(LOG_NOTICE, "Final packet too large!\n");
		return -1;
	}
...
static int check_key(struct dundi_peer *peer, unsigned char *newkey, unsigned char *newsig, unsigned long keycrc32)
{
	...
	char eid_str[20];
	if (option_debug)
		cw_log(LOG_DEBUG, "Expected '%08lx' got '%08lx'\n", peer->them_keycrc32, keycrc32);
	...
	if (!key) {
		cw_log(LOG_NOTICE, "Unable to find key '%s' to decode shared key from '%s'\n",
			peer->outkey, dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		return -1;
	}
	...
	if (!skey) {
		cw_log(LOG_NOTICE, "Unable to find key '%s' to verify shared key from '%s'\n",
			peer->inkey, dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		return -1;
	}
	...
	if (res != 16) {
		if (res >= 0)
			cw_log(LOG_NOTICE, "Weird, key decoded to the wrong size (%d)\n", res);
		return 0;
	}
	/* Decrypted, passes signature */
	cw_log(LOG_DEBUG, "Wow, new key combo passed signature and decrypt!\n");
...
static int handle_command_response(struct dundi_transaction *trans, struct dundi_hdr *hdr, int datalen, int encrypted)
{
	...
	if (datalen) {
		...
		cw_log(LOG_DEBUG, "Got canonical message %d (%d), %d bytes data%s\n", cmd, hdr->oseqno, datalen, final ? " (Final)" : "");
		if (dundi_parse_ies(&ies, bufcpy, datalen) < 0) {
			cw_log(LOG_WARNING, "Failed to parse DUNDI information elements!\n");
			return -1;
		}
	}
	switch(cmd) {
	...
	case DUNDI_COMMAND_DPRESPONSE:
		...
		if (ies.cause < 1) {
			...
			cw_log(LOG_DEBUG, "Looks like success of some sort (%d), %d answers\n", ies.cause, ies.anscount);
				...
				if (trans->parent && trans->parent->dr) {
					...
					for (x=0;x<ies.anscount;x++) {
						if (trans->parent->respcount < trans->parent->maxcount) {
							...
							cw_log(LOG_NOTICE, "Dropping excessive answers to request for %s@%s\n",
								trans->parent->number, trans->parent->dcontext);
					}
	...
	case DUNDI_COMMAND_EIDRESPONSE:
		...
		if (ies.cause < 1) {
			/* Success of some sort */
			cw_log(LOG_DEBUG, "Looks like success of some sort (%d)\n", ies.cause);
			if (cw_test_flag(trans, FLAG_ENCRYPT)) {
				authpass = encrypted;
			} else 
				authpass = 1;
	...
	case DUNDI_COMMAND_REGRESPONSE:
		...
		if (ies.cause < 1) {
			...
			if (!hasauth) {
				cw_log(LOG_NOTICE, "Reponse to register not authorized!\n");
				if (!final) {
				...
				cw_log(LOG_DEBUG, "Yay, we've registered as '%s' to '%s'\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &trans->us_eid),
		...
		if (!encrypted) {
			...
			if (cw_test_flag(trans, FLAG_ENCRYPT) && ies.encblock && ies.enclen) {
				...
				if (dhdr) {
					...
					cw_log(LOG_DEBUG, "Ouch, decrypt failed :(\n");
			}
		}
		if (!final) {
			/* Turn off encryption */
...
static int ack_trans(struct dundi_transaction *trans, int iseqno)
{
	...
	while(pack) {
		if ((pack->h->oseqno + 1) % 255 == iseqno) {
			...
			if (trans->lasttrans) {
				cw_log(LOG_WARNING, "Whoa, there was still a last trans?\n");
...
static int handle_frame(struct dundi_hdr *h, struct sockaddr_in *sin, int datalen)
{
	...
	if (h->oseqno == trans->iseqno) {
		...
		cw_log(LOG_DEBUG, "Dropping packet out of window!\n");
	}
	return 0;
}
...
static int socket_read(int *id, int fd, short events, void *cbdata)
{
	...
	if (res < sizeof(struct dundi_hdr)) {
		cw_log(LOG_WARNING, "midget packet received (%d of %d min)\n", res, (int)sizeof(struct dundi_hdr));
		return 1;
	}
	buf[res] = '\0';
...
static void *network_thread(void *ignore)
{
	...
	cw_io_add(io, netsocket, socket_read, CW_IO_IN, NULL);
	for(;;) {
		/* 10s select timeout */
...
{
	cw_pthread_create(&netthreadid, NULL, network_thread, NULL);
	cw_pthread_create(&precachethreadid, NULL, process_precache, NULL);
	return 0;
}
...
static int dundi_xmit(struct dundi_packet *pack)
{
	...
	if (res < 0) {
		cw_log(LOG_WARNING, "Failed to transmit to '%s:%d': %s\n", 
	...
	if (cw_test_flag(trans, FLAG_ISREG | FLAG_ISQUAL | FLAG_STOREHIST)) {
		...
		while (peer) {
			...
			if (peer->qualtrans == trans) {
				if (fromtimeout) {
					if (peer->lastms > -1)
						cw_log(LOG_NOTICE, "Peer '%s' has become UNREACHABLE!\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
					peer->lastms = -1;
				} else {
					...
					if (ms < peer->maxms) {
						if ((peer->lastms >= peer->maxms) || (peer->lastms < 0))
							cw_log(LOG_NOTICE, "Peer '%s' has become REACHABLE!\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
					} else if (peer->lastms < peer->maxms) {
						cw_log(LOG_NOTICE, "Peer '%s' has become TOO LAGGED (%d ms)\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid), ms);
					}
					peer->lastms = ms;
				}
				peer->qualtrans = NULL;
...
static int dundi_rexmit(void *data)
{
	...
	if (pack->retrans < 1) {
		...
		if (!cw_test_flag(pack->parent, FLAG_ISQUAL))
			cw_log(LOG_NOTICE, "Max retries exceeded to host '%s:%d' msg %d on call %d\n", 
				cw_inet_ntoa(iabuf, sizeof(iabuf), pack->parent->addr.sin_addr), 
...
static int dundi_send(struct dundi_transaction *trans, int cmdresp, int flags, int final, struct dundi_ie_data *ied)
{
	...
	if (pack) {
		...
		if (!res) 
			res = dundi_xmit(pack);
		if (res)
			cw_log(LOG_NOTICE, "Failed to send packet to '%s'\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &trans->them_eid));
...
static int do_autokill(void *data)
{
	...
	char eid_str[20];
	cw_log(LOG_NOTICE, "Transaction to '%s' took too long to ACK, destroying\n", 
		dundi_eid_to_str(eid_str, sizeof(eid_str), &trans->them_eid));
	cw_mutex_lock(&peerlock);
	trans->autokillid = -1;
...
static int dundi_discover(struct dundi_transaction *trans)
{
	...
	int x;
	if (!trans->parent) {
		cw_log(LOG_WARNING, "Tried to discover a transaction with no parent?!?\n");
		return -1;
	}
	memset(&ied, 0, sizeof(ied));
...
static int dundi_query(struct dundi_transaction *trans)
{
	...
	int x;
	if (!trans->parent) {
		cw_log(LOG_WARNING, "Tried to query a transaction with no parent?!?\n");
		return -1;
	}
	memset(&ied, 0, sizeof(ied));
	dundi_ie_append_short(&ied, DUNDI_IE_VERSION, DUNDI_DEFAULT_VERSION);
...
static int precache_transactions(struct dundi_request *dr, struct dundi_mapping *maps, int mapcount, int *expiration, int *foundanswers)
{
	...
	while(trans) {
		if (trans->thread)
			cw_log(LOG_WARNING, "This shouldn't happen, really...\n");
		trans->thread = 1;
		trans = trans->next;
	}
	cw_mutex_unlock(&peerlock);
...
static int append_transaction(struct dundi_request *dr, struct dundi_peer *p, int ttl, dundi_eid *avoid[])
{
	...
	if (cw_strlen_zero(dr->number))
		cw_log(LOG_DEBUG, "Will query peer '%s' for '%s' (context '%s')\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &p->eid), dundi_eid_to_str(eid_str2, sizeof(eid_str2), &dr->query_eid), dr->dcontext);
	else
		cw_log(LOG_DEBUG, "Will query peer '%s' for '%s@%s'\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &p->eid), dr->number, dr->dcontext);
...
static int register_request(struct dundi_request *dr, struct dundi_request **pending)
{
	...
	while(cur) {
		if (option_debug)
			cw_log(LOG_DEBUG, "Checking '%s@%s' vs '%s@%s'\n", cur->dcontext, cur->number,
		...
		if (!strcasecmp(cur->dcontext, dr->dcontext) &&
			...
			(!dundi_eid_cmp(&cur->root_eid, &dr->root_eid) || (cur->crc32 == dr->crc32))) {
				cw_log(LOG_DEBUG, "Found existing query for '%s@%s' for '%s' crc '%08lx'\n", 
	...
	if (!res) {
		cw_log(LOG_DEBUG, "Registering request for '%s@%s' on behalf of '%s' crc '%08lx'\n", 
...
static int dundi_lookup_internal(struct dundi_result *result, int maxret, struct cw_channel *chan, const char *dcontext, const char *number, int ttl, int blockempty, struct dundi_hint_metadata *hmd, int *expiration, int cbypass, int modeselect, dundi_eid *skip, dundi_eid *avoid[], int direct[])
{
	...
	memset(&dr, 0, sizeof(dr));
	if (pipe(dr.pfds)) {
		cw_log(LOG_WARNING, "pipe failed: %s\n" , strerror(errno));
		return -1;
	}
	dr.dr = result;
	dr.hmd = hmd;
	...
	if (res) {
		...
		if (rooteid && !dundi_eid_cmp(&dr.root_eid, &pending->root_eid)) {
			...
			cw_log(LOG_DEBUG, "Oooh, duplicate request for '%s@%s' for '%s'\n",
			...
			cw_log(LOG_DEBUG, "Waiting for similar request for '%s@%s' for '%s'\n",
	...
	if (chan && chan->_softhangup)
		cw_log(LOG_DEBUG, "Hrm, '%s' hungup before their query for %s@%s finished\n", chan->name, dr.number, dr.dcontext);
	cancel_request(&dr);
	unregister_request(&dr);
	res = dr.respcount;
...
static void dundi_precache_full(void)
{
	...
	cur = mappings;
	while(cur) {
		cw_log(LOG_NOTICE, "Should precache context '%s'\n", cur->dcontext);
		cw_lock_contexts();
...
static int dundi_precache_internal(const char *context, const char *number, int ttl, dundi_eid *avoids[])
{
	...
	if (!context)
		context = "e164";
	cw_log(LOG_DEBUG, "Precache internal (%s@%s)!\n", number, context);
	...
	if (foundanswers) {
		if (dr.expiration > 0) 
			...
			cw_log(LOG_NOTICE, "Weird, expiration = %d, but need to precache for %s@%s?!\n", dr.expiration, dr.number, dr.dcontext);
	}
	start = cw_tvnow();
...
static int dundi_lookup_exec(struct cw_channel *chan, int argc, char **argv)
{
	char *context;
	int results;
	int x;
	...
	if (!dep_warning) {
		cw_log(LOG_WARNING, "This application has been deprecated in favor of the DUNDILOOKUP dialplan function.\n");
		dep_warning = 1;
	}
	...
	if (success) {
		pbx_builtin_setvar_helper(chan, "SUCCESS", dr[x].dest);
	} else {
		pbx_builtin_setvar_helper(chan, "FAIL", dr[x].dest);
	}
...
static char *dundifunc_read(struct cw_channel *chan, int argc, char **argv, char *buf, size_t len)
{
	char *context;
	int results;
	int x;
	...
	if (map) {
		...
		if ((x == 1) && cw_strlen_zero(fields[0])) {
			...
			if ((sscanf(fields[1], "%d", &map->weight) == 1) && (map->weight >= 0) && (map->weight < 60000)) {
				...
				cw_log(LOG_WARNING, "Invalid weight '%s' specified, deleting entry '%s/%s'\n", fields[1], map->dcontext, map->lcontext);
			}
			for (y=4;y<x;y++) {
				if (!strcasecmp(fields[y], "nounsolicited"))
					...
					cw_log(LOG_WARNING, "Don't know anything about option '%s'\n", fields[y]);
			}
		} else 
			cw_log(LOG_WARNING, "Expected at least %d arguments in map, but got only %d\n", 4, x);
	}
}
...
static int do_register(void *data)
{
	...
	cw_log(LOG_DEBUG, "Register us as '%s' to '%s'\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->us_eid), dundi_eid_to_str(eid_str2, sizeof(eid_str2), &peer->eid));
	...
	if (peer->regtrans) {
		...
		cw_log(LOG_NOTICE, "Unable to create new transaction for registering to '%s'!\n", dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
...
static void build_peer(dundi_eid *eid, struct cw_variable *v, int *globalpcmode)
{
	...
	if (peer) {
			...
			if (!strcasecmp(v->name, "inkey")) {
				...
				if (!strcasecmp(v->value, "dynamic")) {
					...
					if (hp) {
						...
						cw_log(LOG_WARNING, "Unable to find host '%s' at line %d\n", v->value, v->lineno);
						peer->dead = 1;
					}
				}
				...
				if (!dundi_str_to_eid(&testeid, v->value))
					...
					cw_log(LOG_WARNING, "'%s' is not a valid DUNDi Entity Identifier at line %d\n", v->value, v->lineno);
				...
				if (!strcasecmp(v->value, "primary"))
					...
					cw_log(LOG_WARNING, "'%s' is not a valid order, should be primary, secondary, tertiary or quartiary at line %d\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "no")) {
					...
					cw_log(LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of dundi.conf\n", 
				...
				if (!strcasecmp(v->value, "inbound"))
					...
					cw_log(LOG_WARNING, "Unknown model '%s', should be 'none', 'outbound', 'inbound', or 'symmetric' at line %d\n", 
						v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "inbound"))
					...
					cw_log(LOG_WARNING, "Unknown pcmodel '%s', should be 'none', 'outbound', 'inbound', or 'symmetric' at line %d\n", 
						v->value, v->lineno);
				}
			}
			v = v->next;
		...
		if (!peer->model && !peer->pcmodel) {
			cw_log(LOG_WARNING, "Peer '%s' lacks a model or pcmodel, discarding!\n", 
				dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
			peer->dead = 1;
		} else if ((peer->model & DUNDI_MODEL_INBOUND) && (peer->pcmodel & DUNDI_MODEL_OUTBOUND)) {
			cw_log(LOG_WARNING, "Peer '%s' may not be both inbound/symmetric model and outbound/symmetric precache model, discarding!\n", 
				dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
			peer->dead = 1;
		} else if ((peer->model & DUNDI_MODEL_OUTBOUND) && (peer->pcmodel & DUNDI_MODEL_INBOUND)) {
			cw_log(LOG_WARNING, "Peer '%s' may not be both outbound/symmetric model and inbound/symmetric precache model, discarding!\n", 
				dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
			peer->dead = 1;
		} else if (peer->include && !(peer->model & DUNDI_MODEL_OUTBOUND) && !(peer->pcmodel & DUNDI_MODEL_INBOUND)) {
			cw_log(LOG_WARNING, "Peer '%s' is supposed to be included in outbound searches but isn't an outbound peer or inbound precache!\n", 
				dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		} else if (peer->permit && !(peer->model & DUNDI_MODEL_INBOUND) && !(peer->pcmodel & DUNDI_MODEL_OUTBOUND)) {
			cw_log(LOG_WARNING, "Peer '%s' is supposed to have permission for some inbound searches but isn't an inbound peer or outbound precache!\n", 
				dundi_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		} else { 
			if (needregister) {
...
static int dundi_helper(struct cw_channel *chan, const char *context, const char *exten, int priority, const char *data, int flag)
{
	...
	if (!strncasecmp(context, "proc-", 5)) {
		if (!chan) {	
			cw_log(LOG_NOTICE, "Can't use Proc mode without a channel!\n");
			return -1;
		}
		...
		if (!strcasecmp(exten, "s")) {
			...
			if (!exten || cw_strlen_zero(exten))
				exten = chan->exten;
			if (!exten || cw_strlen_zero(exten)) {	
				cw_log(LOG_WARNING, "Called in Proc mode with no ARG1 or PROC_EXTEN?\n");
				return -1;
			}
		}
...
static int set_config(char *config_file, struct sockaddr_in* sin)
{
	...
	if (!gethostname(hn, sizeof(hn)-1)) {
		...
		if (hp) {
			...
			cw_log(LOG_WARNING, "Unable to look up host '%s'\n", hn);
	} else
		cw_log(LOG_WARNING, "Unable to get host name!\n");
	cw_mutex_lock(&peerlock);
	reset_global_eid();
	global_storehistory = 0;
	...
	while(v) {
		if (!strcasecmp(v->name, "port")){ 
			...
			if(last_port==0){
				...
				cw_log(LOG_WARNING, "change to port ignored until next callweaver re-start\n");
			...
			if (hp) {
				...
				cw_log(LOG_WARNING, "Invalid host/IP '%s'\n", v->value);
			...
			if ((sscanf(v->value, "%d", &x) == 1) && (x > 0) && (x < DUNDI_DEFAULT_TTL)) {
				...
				cw_log(LOG_WARNING, "'%s' is not a valid TTL at line %d, must be number from 1 to %d\n",
			...
			if (!dundi_str_to_eid(&testeid, v->value))
				...
				cw_log(LOG_WARNING, "Invalid global endpoint identifier '%s' at line %d\n", v->value, v->lineno);
			...
			if ((sscanf(v->value, "%d", &x) == 1)) {
				...
				cw_log(LOG_WARNING, "'%s' is not a valid cache time at line %d. Using default value '%d'.\n",
	...
	while(cat) {
		if (strcasecmp(cat, "general") && strcasecmp(cat, "mappings")) {
			...
			if (!dundi_str_to_eid(&testeid, cat))
				...
				cw_log(LOG_NOTICE, "Ignoring invalid EID entry '%s'\n", cat);
		}
...
int unload_module(void)
{
	...
	cw_unregister_switch(&dundi_switch);
	res |= cw_unregister_function(dundi_func);
	res |= cw_unregister_application(dundi_app);
	return res;
}
...
int load_module(void)
{
	...
	if (bind(netsocket,(struct sockaddr *)&sin, sizeof(sin))) {
		cw_log(LOG_ERROR, "Unable to bind to %s port %d: %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port), strerror(errno));
		return -1;
	}
	...
	if (cw_register_switch(&dundi_switch))
		cw_log(LOG_ERROR, "Unable to register DUNDi switch\n");
	dundi_app = cw_register_application(dundi_lookup_name, dundi_lookup_exec, dundi_lookup_synopsis, dundi_lookup_syntax, dundi_lookup_descrip);
	dundi_func = cw_register_function(dundifunc_name, dundifunc_read, NULL, dundifunc_synopsis, dundifunc_syntax, dundifunc_desc); 
