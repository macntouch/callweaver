...
CALLWEAVER_FILE_VERSION("$HeadURL: https://svn.callweaver.org/callweaver/branches/rel/1.2/corelib/callweaver.c $", "$Revision: 5231 $")
...
#define WELCOME_MESSAGE cw_verbose("%s http://www.callweaver.org - The True Open Source PBX\n", cw_version_string); \
		cw_verbose( "=========================================================================\n")
...
char cw_config_CW_HISTORY_FILE[CW_CONFIG_MAX_PATH] = "";
...
void cw_register_file_version(const char *file, const char *version)
{
    ...
    if (new)
    {
        ...
        version = cw_skip_blanks(version);
...
static void quit_handler(int num, int nice, int safeshutdown, int restart)
{
    ...
    {
        if (!cw_strlen_zero(cw_config_CW_HISTORY_FILE))
            cw_rl_write_history(cw_config_CW_HISTORY_FILE);
    }
    if (option_verbose)
...
static void cw_remotecontrol(char * data)
{
    ...
    if (!cw_strlen_zero(cw_config_CW_HISTORY_FILE))
        cw_rl_read_history(cw_config_CW_HISTORY_FILE);
...
static void cw_readconfig(void)
{
    ...
    if (getenv("HOME"))
        snprintf(cw_config_CW_HISTORY_FILE, sizeof(cw_config_CW_HISTORY_FILE), "%s/.callweaver_history", getenv("HOME"));
    ...
    while (v)
    {
        ...
        else if (!strcasecmp(v->name, "cwclihistory"))
        {
            if (!cw_strlen_zero(v->value)) {
                if (v->value[0] == '/') {
                    cw_copy_string(cw_config_CW_HISTORY_FILE, v->value, sizeof(cw_config_CW_HISTORY_FILE));
                } else {
                    snprintf(cw_config_CW_HISTORY_FILE, sizeof(cw_config_CW_HISTORY_FILE), "%s/%s", cw_config_CW_VAR_DIR, v->value);
                }
            }
