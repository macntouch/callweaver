...
CALLWEAVER_FILE_VERSION("$HeadURL: https://svn.callweaver.org/callweaver/branches/rel/1.2/channels/chan_agent.c $", "$Revision: 5073 $")
...
static int agents_show(int fd, int argc, char **argv)
{
	...
	int showflag = 0;			/* 0=all, 1=online, 2=offline, 3=like */
	int showagent;
	...
	while(p) {
		...
		if (p->pending) {
			...
			if(showflag==0)
				showagent=1;
			else if(showflag==1 && (p->chan || !cw_strlen_zero(p->loginchan)))
				showagent=1;
			else if(showflag==2 && cw_strlen_zero(p->loginchan))
				showagent=1;
			else if(showflag==3 && (strstr(p->agent,argv[3]) || strstr(username,argv[3]) || strstr(location,argv[3])))
				showagent=1;
...
"Usage: show agents [online|offline|like {pattern}]\n"
"       Provides summary information on agents.\n"
"       If 'online' is specified, only online agents will be shown.\n"
"       If 'offline' is specified, only offline agents will be shown.\n"
"       If 'like' is specified, only agents matching 'pattern' will be shown.\n";
