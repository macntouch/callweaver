 ...
 * 
 * Implementation of RFC 3261 - without S/MIME support
...
CALLWEAVER_FILE_VERSION("$HeadURL: https://svn.callweaver.org/callweaver/branches/rel/1.2/channels/chan_sip.c $", "$Revision: 5358 $")
...
#define OPENSSL_NO_KRB5                /* to prevent compile error */
...
#define DEFAULT_RFC_TIMER_T2       4000        /* Default Maximum retransmit interval for non-INVITEs in ms (RFC3261 requires 4s) */
...
#define DEFAULT_SIP_TLS_PORT	5061	/* From RFC 3261 */
#define DEFAULT_PASSWORD "callweaver"
#define DEFAULT_ENTROPY		"/dev/urandom"
#define CIPHER_LIST	"ALL"

static char trustcerts_file[MAX_PATH_LEN];
static char servercert_file[MAX_PATH_LEN];
static char serverkey_file[MAX_PATH_LEN];
static char serverkey_password[MAX_PATH_LEN] = DEFAULT_PASSWORD;
static char dh512param_file[MAX_PATH_LEN];
static char dh1024param_file[MAX_PATH_LEN];
#endif
...
#ifdef SIP_TCP_SUPPORT
    SSL *ssl;					/* SSL object for TLS connection */
    int sockfd;                 /* socket fd used by this SIP channel*/
    char transport[4];          /* transport protocol: UDP, TCP or TLS */
#endif        
...
#ifdef SIP_TCP_SUPPORT
    SSL *ssl;				/* SSL object for TLS connection */
    int sockfd;				/* Connection socket is saved to here */
    char transport[4];		/* transport protocol: UDP, TCP or TLS */
#endif
    /* Qualification */
...
#ifdef SIP_TCP_SUPPORT
static int siptcpsock = -1;		/* TCP listening socket */
static int siptlssock = -1;		/* TLS listening socket */
static SSL_CTX *tlsctx = NULL;	/* SSL Context for TLS */
static struct sockaddr_in tlsbindaddr;
#endif
...
#endif
static struct sip_peer *build_peer(const char *name, struct cw_variable *v, int realtime, struct cw_config *cfg);
static struct sip_user *build_user(const char *name, struct cw_variable *v, int realtime, struct cw_config *cfg);
...
static int sip_call(struct cw_channel *chan, char *dest, int timeout);
static int sip_hangup(struct cw_channel *chan);
static int sip_answer(struct cw_channel *chan);
static struct cw_frame *sip_read(struct cw_channel *chan);
static int sip_write(struct cw_channel *chan, struct cw_frame *frame);
static int sip_indicate(struct cw_channel *chan, int condition);
static int sip_transfer(struct cw_channel *chan, const char *dest);
static int sip_fixup(struct cw_channel *oldchan, struct cw_channel *newchan);
static int sip_senddigit(struct cw_channel *chan, char digit);
...
#ifdef SIP_TCP_SUPPORT
	if ((p->sockfd > 0) && (p->sockfd != sipsock)) {   /* This is TCP */
			if (p->ssl) {		/* TLS write */
				if( sipdebug )
					cw_verbose("TLS write: fd %d\n", p->sockfd);

				struct sip_peer *peer = NULL;
				res = 0;
				while (len > res) {
					res += SSL_write(p->ssl, data + res, len - res);
					switch (SSL_get_error(p->ssl, res)) {
						case SSL_ERROR_NONE:
							break;
						default:
							cw_log(LOG_ERROR, "SSL write error\n");
							SSL_clear(p->ssl);
							SSL_free(p->ssl);
							p->ssl = NULL;
							p->sockfd = -1;
				...
				if( sipdebug )
					cw_verbose("TCP write: fd %d\n", p->sockfd);
				res = write(p->sockfd, data, len);
			}
	} else {
		if( sipdebug )
			cw_verbose("UDP write: fd %d\n", p->sockfd);
#endif
...
#ifdef SIP_TCP_SUPPORT
		  snprintf(buf, len, "SIP/2.0/%s %s:%d;branch=z9hG4bK%08x;rport", p->transport, cw_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport, p->branch);
#else
...
#ifdef SIP_TCP_SUPPORT
        snprintf(buf, len, "SIP/2.0/%s %s:%d;branch=z9hG4bK%08x", p->transport, cw_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport, p->branch);
#else      
...
static int retrans_pkt(void *data)
{
    ...
    if (pkt->owner && pkt->method != SIP_OPTIONS)
    {
        if (cw_test_flag(pkt, FLAG_FATAL) || sipdebug)    /* Tell us if it's critical or if we're debugging */
            cw_log(LOG_WARNING, "Maximum retries exceeded on transmission %s for seqno %d (%s %s) SIP Timer T1 : %d\n", pkt->owner->callid, pkt->seqno, (cw_test_flag(pkt, FLAG_FATAL)) ? "Critical" : "Non-critical", (cw_test_flag(pkt, FLAG_RESPONSE)) ? "Response" : "Request", pkt->owner->timer_t1);
    }
    else
    {
    ...
    while (cur)
    {
        if ((cur->seqno == seqno) && ((cw_test_flag(cur, FLAG_RESPONSE)) == resp)
            ...
            ((cw_test_flag(cur, FLAG_RESPONSE)) || 
             (!strncasecmp(msg, cur->data, strlen(msg)) && (!cur->data[strlen(msg)] || isspace(cur->data[strlen(msg)])))))
        {
...
#ifdef SIP_TCP_SUPPORT
static int cleanup_tcp_connection(int fd, SSL *ssl)
{
	int found = 0;
	struct sip_pvt *p = NULL;
	
	if (fd == -1) {
		cw_log(LOG_ERROR, "Tried to free up sockfd -1\n");
		return -1;
	}

//	CWOBJ_CONTAINER_RDLOCK(&peerl);
	CWOBJ_CONTAINER_TRAVERSE(&peerl, !found, {
		if (iterator->sockfd == fd) {
			found = 1;
			if (iterator->ssl) {
				SSL_clear(iterator->ssl);
				SSL_free(iterator->ssl);
			}
			else
				close(iterator->sockfd);
			iterator->ssl = NULL;
			iterator->sockfd = -1;
/*			cw_copy_string(iterator->transport, "UDP", sizeof(iterator->transport)); */
		}
	} );
//	CWOBJ_CONTAINER_UNLOCK(&peerl);

	/* close tcp fd and free ssl in sip_pvt */
	...
	while (p) {
		if (p->sockfd == fd) {
			...
			p->ssl = NULL;
			p->sockfd = -1;
/*			cw_copy_string(p->transport, "UDP", sizeof(p->transport)); */
			cw_mutex_unlock(&p->lock);
			break;
		}
		p = p->next;
	...
	cw_log(LOG_DEBUG, "Cleaned up tcp connection for fd %d, ssl %p\n", fd, ssl);
	return 0;
}

static int tcptls_connect(struct sip_pvt *p)
{
	int fd = -1;
	...
	if (!strncasecmp(p->transport, "UDP", 3)) {
		return sipsock;
	}

	if ( (fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		cw_log(LOG_WARNING, "TCP can't create socket : %s\n", strerror(errno));
		return -1;
	}

	/* bind local protocol address to socket */
	bzero(&myaddr, sizeof(struct sockaddr_in));
	myaddr.sin_family = AF_INET;
	memcpy(&myaddr.sin_addr, &p->ourip, sizeof(p->ourip));
	myaddr.sin_port = htons(0);       /* any port is OK? */

	if (bind(fd, (struct sockaddr *)&myaddr, sizeof(struct sockaddr_in)) < 0) {
		cw_log(LOG_WARNING, "TCP failed to bind : %s\n", strerror(errno));
		return -1;
	}

	/* start 3-way hand shake with the peer */
	if (connect(fd, (struct sockaddr *) &p->sa, sizeof(struct sockaddr_in)) < 0) {
		cw_log(LOG_WARNING, "TCP can't connect to %s:%d, error %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr), ntohs(p->sa.sin_port), strerror(errno));
		return -1;
	}

	if (sip_debug_test_pvt(p))
		cw_verbose(VERBOSE_PREFIX_2 "Successfuly TCP connected fd %d to %s:%d\n", fd, cw_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr), ntohs(p->sa.sin_port));

	p->sockfd = fd;

	/* If TLS, do TLS handshake */
	if ((fd > -1) && !strncasecmp(p->transport, "TLS", sizeof("TLS"))) {
		...
		if(SSL_connect(ssl) <= 0) {
			cw_log(LOG_ERROR, "SSL_connect error");
			SSL_clear(ssl);
			SSL_free(ssl);
			close(fd);
			p->ssl = NULL;
		...
		cw_log(LOG_DEBUG, "New TLS connection is opened\n");
		p->ssl = ssl;
	}
...
static struct sip_peer *realtime_peer(const char *peername, struct sockaddr_in *sin)
{
    ...
    peer = build_peer(newpeername, var, !cw_test_flag((&global_flags_page2), SIP_PAGE2_RTCACHEFRIENDS), NULL);
    if (!peer)
    {
        cw_variables_destroy(var);
        return (struct sip_peer *) NULL;
...
static struct sip_user *realtime_user(const char *username)
{
    ...
    user = build_user(username, var, !cw_test_flag((&global_flags_page2), SIP_PAGE2_RTCACHEFRIENDS), NULL);
...
#ifdef SIP_TCP_SUPPORT
    r->sockfd = peer->sockfd;
    r->ssl = peer->ssl;
    cw_copy_string(r->transport, peer->transport, sizeof(r->transport));
#endif
...
static int create_addr(struct sip_pvt *dialog, char *opeer)
{
    ...
    else
    {
...
#ifdef SIP_TCP_SUPPORT
        cw_copy_string(transport, DEFAULT_TRANSPORT, sizeof(DEFAULT_TRANSPORT));
#endif /* SIP_TCP_SUPPORT */
        ...
        if (srvlookup)
        {
            ...
            int tportno;
            int ret;
            char srvTransport[256];
            snprintf(service, sizeof(service), "_sip._udp.%s", peer);
            ret = cw_get_srv(NULL, host, sizeof(host), &tportno, service);
#ifdef SIP_TCP_SUPPORT
			cw_copy_string(srvTransport, "UDP", sizeof(srvTransport));
			if( ret <= 0 ){
				snprintf(service, sizeof(service), "_sip._tcp.%s", peer);
				ret = cw_get_srv(NULL, host, sizeof(host), &tportno, service);
   			cw_copy_string(srvTransport, "TCP", sizeof(srvTransport));
			}
			if( ret <= 0 ){
				snprintf(service, sizeof(service), "_sip._tls.%s", peer);
				ret = cw_get_srv(NULL, host, sizeof(host), &tportno, service);
				cw_copy_string(srvTransport, "TLS", sizeof(srvTransport));
			}
			if( ret <= 0 ){
				snprintf(service, sizeof(service), "_sipinternal._udp.%s", peer);
				ret = cw_get_srv(NULL, host, sizeof(host), &tportno, service);
				cw_copy_string(srvTransport, "UDP", sizeof(srvTransport));
			}
			if( ret <= 0 ){
				snprintf(service, sizeof(service), "_sipinternal._tcp.%s", peer);
				ret = cw_get_srv(NULL, host, sizeof(host), &tportno, service);
				cw_copy_string(srvTransport, "TCP", sizeof(srvTransport));
			}
			if( ret <= 0 ){
				snprintf(service, sizeof(service), "_sipinternal._tls.%s", peer);
				ret = cw_get_srv(NULL, host, sizeof(host), &tportno, service);
				cw_copy_string(srvTransport, "TLS", sizeof(srvTransport));
			}
#endif /* SIP_TCP_SUPPORT */
            if (ret > 0)
            {
                ...
                portno = tportno;
#ifdef SIP_TCP_SUPPORT
				cw_copy_string(transport, srvTransport, sizeof(transport));

				if( sipdebug )
					cw_verbose( "Found information about services _sip._xxx.%s (%s:%d; transport=%s)\n", peer, hostn, portno, transport );

#endif /* SIP_TCP_SUPPORT */
			}else{
				cw_log( LOG_WARNING, "Can not get information about services _sip._xxx.%s\n", peer );
 			}
...
#ifdef SIP_TCP_SUPPORT
            cw_copy_string(dialog->transport, transport, sizeof(dialog->transport));
#endif /* SIP_TCP_SUPPORT */
            return 0;
        }
        else
			     ...
			     unsigned long ssrc, int inbound)
{
	...
	if (cw_srtp_policy_set_suite(policy, suite_val)) {
		cw_log(LOG_WARNING, "Could not set remote SRTP suite\n");
		return -1;
	}
			   ...
			   unsigned char *remote_key)
{
	...
	if (cw_rtp_add_srtp_policy(p->rtp, local_policy)) {
		cw_log(LOG_WARNING, "Could not set local SRTP policy\n");
		goto err;
	}

	if (cw_rtp_add_srtp_policy(p->rtp, remote_policy)) {
		cw_log(LOG_WARNING, "Could not set remote SRTP policy\n");
		goto err;
	}


	cw_log(LOG_DEBUG, "SRTP policy activated\n");
	res = 0;
...
static int process_crypto(struct sip_pvt *p, const char *attr)
{
	...
	if (!tag || !suite) {
		cw_log(LOG_WARNING, "Unrecognized a=%s", attr);
		return -1;
	}

	if (session_params) {
		cw_log(LOG_WARNING, "Unsupported crypto parameters: %s",
			session_params);
		return -1;
	}
	...
	if (!strcmp(suite, "AES_CM_128_HMAC_SHA1_80")) {
		...
		cw_log(LOG_WARNING, "Unsupported crypto suite: %s",
			suite);
		return -1;
	}
	...
	while ((key_param = strsep(&key_params, ";"))) {
		...
		if (!strcmp(method, "inline")) {
			...
			if (lifetime) {
				cw_log(LOG_NOTICE, "Crypto life time unsupported: %s\n",
					attr);
				continue;
			}

/* 			if (info || strncmp(lifetime, "2^", 2)) { */
/* 				cw_log(LOG_NOTICE, "MKI unsupported: %s\n", */
/* 					attr); */
/* 				continue; */
/* 			} */
	...
	if (!found) {
		cw_log(LOG_NOTICE, "SRTP crypto offer not acceptable\n");
		return -1;
	}
	...
	if (key_len != SRTP_MASTER_LEN) {
		cw_log(LOG_WARNING, "SRTP sdescriptions key %d != %d\n",
...
static int sip_call(struct cw_channel *chan, char *dest, int timeout)
{
    ...
    p = chan->tech_pvt;
    if ((chan->_state != CW_STATE_DOWN) && (chan->_state != CW_STATE_RESERVED))
    {
    ...
    CW_LIST_TRAVERSE(headp,current,entries)
    {
        ...
        else if (!strncasecmp(cw_var_name(current), "SIP_SRTP_SDES", strlen("SIP_SRTP_SDES")))
        {
				if (!cw_srtp_is_registered()) {
				cw_log(LOG_WARNING, "SIP_SRTP_SDES set but SRTP is not available\n");
				return -1;
				}

				if (!p->srtp) {
					if (setup_crypto(p) < 0) {
					cw_log(LOG_WARNING, "SIP SRTP sdes setup failed\n");
					return -1;
					}
				}
#endif
    ...
    if (res != -1)
    {
        p->callingpres = chan->cid.cid_pres;
...
/*#ifdef SIP_TCP_SUPPORT
	if ((p->sockfd > 0) && (p->sockfd != sipsock))
		close(p->sockfd);
	if (p->ssl)
		SSL_free(p->ssl);
#endif*/
    ...
    if (!cw_test_flag(p, SIP_ALREADYGONE) && !cw_strlen_zero(p->initreq.data))
    {
		...
		if (!cw_test_flag(p, SIP_CAN_BYE)) {
            ...
            else
            {
                ...
                if (chan->hangupcause && ((res = hangup_cause2sip(chan->hangupcause))))
...
static int sip_rtp_write(struct cw_channel *chan, struct cw_frame *frame, int *faxdetect)
{
    struct sip_pvt *p = chan->tech_pvt;
    int res = 0;
    ...
    {
    case CW_FRAME_VOICE:
        if (!(frame->subclass & chan->nativeformats))
        {
            cw_log(LOG_WARNING, "Asked to transmit frame type %d, while native formats is %d (read/write = %d/%d)\n",
                frame->subclass, chan->nativeformats, chan->readformat, chan->writeformat);
            return 0;
        }
        if (p)
        {
            ...
            if (p->rtp)
            {
                ...
                if ((chan->_state != CW_STATE_UP) && !cw_test_flag(p, SIP_PROGRESS_SENT) && !cw_test_flag(p, SIP_OUTGOING))
                {
...
static int sip_write(struct cw_channel *chan, struct cw_frame *frame)
{
    ...
    res = sip_rtp_write(chan, frame, &faxdetected);
...
static int sip_indicate(struct cw_channel *chan, int condition)
{
    ...
    case CW_CONTROL_BUSY:
        if (chan->_state != CW_STATE_UP)
        {
            ...
            cw_softhangup_nolock(chan, CW_SOFTHANGUP_DEV);
            break;
        }
        res = -1;
        break;
    ...
    case CW_CONTROL_FLASH:    /* Send a hook flash */
        ...
        case SIP_DTMF_RFC2833:
            if (p->rtp) {
                ...
                cw_playtones_start(chan, 0, "!0/80", 0);
            }
            break;
        case SIP_DTMF_INBAND:
            res = -1;
...
static struct cw_frame *sip_rtp_read(struct cw_channel *chan, struct sip_pvt *p, int *faxdetect)
{
...
/*
    ...
    cw_log(LOG_DEBUG,"sip_read - fdno %d - udptl is active: %d - udptl %d \n",
                chan->fdno, p->udptl_active, ( p->udptl ) ? 1 : 0 
            );
*/
    switch (chan->fdno)
    {
...
static struct cw_frame *sip_read(struct cw_channel *chan)
{
    ...
    cw_mutex_lock(&p->lock);
    fr = sip_rtp_read(chan, p, &faxdetected);
    time(&p->lastrtprx);
    cw_mutex_unlock(&p->lock);
...
static struct sip_pvt *sip_alloc(char *callid, struct sockaddr_in *sin, int useglobal_nat, const int intended_method)
{
    ...
    p->ssl = NULL;
    p->sockfd = -1;
    cw_copy_string(p->transport, DEFAULT_TRANSPORT, sizeof(p->transport));  /* default transport protocol */
#endif
...
static int process_sdp(struct sip_pvt *p, struct sip_request *req)
{
    ...
    while ((m = get_sdp_iterate(&iterator, req, "m"))[0] != '\0')
    {
        ...
        len = -1;
			...
			if (!strcmp(protocol, "SAVP"))
				...
				cw_log(LOG_WARNING, "Unknown SDP media protocol in offer: %s\n", protocol);
				continue;
			}

			if (len < 0) {
				cw_log(LOG_WARNING, "Unknown SDP media type in offer: %s\n", m);
				continue;
			}
#else
		  int found = 0;
    ...
    {
		cw_log(LOG_WARNING, "Can't provide secure audio requested in SDP offer\n");
		return -2;
	 }
#endif
    if (udptlportno != -1)
    {
        ...
        while ((a = get_sdp_iterate(&iterator, req, "a"))[0] != '\0')
        {
            ...
            else if ((sscanf(a, "T38FaxFillBitRemoval:%d", &x) == 1  &&  x == 1)
                     ||
                     strcmp(a, "T38FaxFillBitRemoval") == 0)
            {
                found = 1;
                cw_log(LOG_DEBUG,"FillBitRemoval: %d\n",x);
                peert38capability |= T38FAX_FILL_BIT_REMOVAL;
            }
            else if ((sscanf(a, "T38FaxTranscodingMMR:%d", &x) == 1  &&  x == 1)
                     ||
                     strcmp(a, "T38FaxTranscodingMMR") == 0)
            {
                found = 1;
                cw_log(LOG_DEBUG,"Transcoding MMR: %d\n",x);
                peert38capability |= T38FAX_TRANSCODING_MMR;
            }
            else if ((sscanf(a, "T38FaxTranscodingJBIG:%d", &x) == 1  &&  x == 1)
                     ||
                     strcmp(a, "T38FaxTranscodingJBIG") == 0)
            {
                found = 1;
...
static void set_destination(struct sip_pvt *p, char *uri)
{
...
#else
	/* Parse uri to h (host), port and transport - uri is already just the part inside the <> */
	/* general form we are expecting is sip[s]:username[:password]@host[:port][;...][;transport=UDP|TCP|TLS][;...] */
#endif /* SIP_TCP_SUPPORT */

    if (debug)
#ifdef SIP_TCP_SUPPORT
        cw_verbose("set_destination: Parsing <%s> for address/port/transport to send to\n", uri);
#else
...
#ifdef SIP_TCP_SUPPORT
	/* Find and parse transport "transport=" */
	t = strstr(uri, "transport=");
	if( t ) {
		t += 10;
		tn = strcspn(t, ";>") + 1;
		if (tn > sizeof(transport)) 
			tn = sizeof(transport);
		cw_copy_string(transport, t, tn);
	}
	else
		cw_copy_string(transport, DEFAULT_TRANSPORT, sizeof(DEFAULT_TRANSPORT));
	...
	if(!strncasecmp(transport, "UDP", 3)) {
		cw_copy_string(p->transport, "UDP", sizeof(p->transport));
	}else if(!strncasecmp(transport, "TCP", 3)) {
		cw_copy_string(p->transport, "TCP", sizeof(p->transport));
	}else if(!strncasecmp(transport, "TLS", 3)) {
		cw_copy_string(p->transport, "TLS", sizeof(p->transport));
	}else {
   ...
   p->sa.sin_port = htons(port);
    ...
    if (debug)
#ifdef SIP_TCP_SUPPORT
        cw_verbose("set_destination: set destination to %s, port %d, transport %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr), port, p->transport);
#else
...
static int add_t38_sdp(struct sip_request *resp, struct sip_pvt *p)
{
    ...
    {
        snprintf(a_modem, sizeof(a_modem), "a=T38FaxFillBitRemoval");
        add_line(resp, a_modem, SIP_DL_DONTCARE);
    }
    if (p->t38jointcapability & T38FAX_TRANSCODING_MMR)
    {
        snprintf(a_modem, sizeof(a_modem), "a=T38FaxTranscodingMMR");
        add_line(resp, a_modem, SIP_DL_DONTCARE);
    }
    if (p->t38jointcapability & T38FAX_TRANSCODING_JBIG)
    {
        snprintf(a_modem, sizeof(a_modem), "a=T38FaxTranscodingJBIG");
        add_line(resp, a_modem, SIP_DL_DONTCARE);
    }
...
#if 0
    snprintf(a_modem, sizeof(a_modem), "a=T38VendorInfo:0 0 0");
    add_line(resp, a_modem, SIP_DL_DONTCARE);
#endif
...
// TCP_ToDo: Hmm, what about stun and TCP?
...
#ifdef SIP_TCP_SUPPORT
        snprintf(p->our_contact, sizeof(p->our_contact), "<sip:%s%s%s:%d;transport=%s>", p->exten, cw_strlen_zero(p->exten) ? "" : "@", cw_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport, p->transport);
#else
        snprintf(p->our_contact, sizeof(p->our_contact), "<sip:%s%s%s:%d;transport=udp>", p->exten, cw_strlen_zero(p->exten) ? "" : "@", cw_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport);
#endif
        else
#ifdef SIP_TCP_SUPPORT
        snprintf(p->our_contact, sizeof(p->our_contact), "<sip:%s%s%s;transport=%s>", p->exten, cw_strlen_zero(p->exten) ? "" : "@", cw_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->transport);
#else
...
#ifdef SIP_TCP_SUPPORT
    if (!cw_strlen_zero(p->fullcontact) && strchr(p->fullcontact, '@') != NULL) 
#else    
...
static int transmit_refer(struct sip_pvt *p, const char *dest)
{
    ...
    if (strncasecmp(of, "sip:", 4) == 0) {	
		of += 4;
	 } else if (strncasecmp(of, "sips:", 5) == 0) {
		of += 5;
	 } else {
		cw_log(LOG_NOTICE, "From address missing 'sip:' or 'sips:', using it anyway\n");
	 }	
...
static int parse_ok_contact(struct sip_pvt *pvt, struct sip_request *req)
{
    ...
    if ( strncasecmp(c, "sip:", 4) == 0 ) {
 		c += 4;
    } else if ( strncasecmp(c, "sips:", 5) == 0 ) {
		c += 5;
    }else {
		cw_log(LOG_NOTICE, "'%s' is not a valid SIP contact (missing 'sip:' or 'sips:') trying to use anyway\n", c);
	 } 
...
static enum parse_register_result parse_register_contact(struct sip_pvt *pvt, struct sip_peer *p, struct sip_request *req)
{
    ...
    */
	...
	if (option_debug > 2 && sipdebug) {
		cw_log(LOG_DEBUG, "Contact header: transport %s\n", t ? t : "");
		cw_log(LOG_DEBUG, "Contact header: q %s\n", q ? q : "");
	}	
#endif
	/* Ditch first ';' */
    n = strchr(c, ';');
...
#ifdef SIP_TCP_SUPPORT
	/* Check a peer has old stalled TCP or TLS connection */
/*	if (p->ssl) {
		SSL_clear(p->ssl);
		SSL_free(p->ssl);
		p->ssl = NULL;
	...
	p->ssl = pvt->ssl;
	if (t) {
		if(!strncasecmp(t, "TCP", 3)) {
			cw_copy_string(p->transport, "TCP", sizeof(p->transport));
		} else if (!strncasecmp(t, "TLS", 3)) {
			cw_copy_string(p->transport, "TLS", sizeof(p->transport));
			if (!pvt->ssl)
				cw_log(LOG_ERROR, "transport tls comes without SSL from peer %s, fd %d\n", p->name, pvt->sockfd);
		} else if (!strncasecmp(t, "UDP", 3)) {
			cw_copy_string(p->transport, "UDP", sizeof(p->transport));
		} else {
			cw_log(LOG_ERROR, "Unsupported transport %s comes from peer %s, fd %d\n", t, p->name, pvt->sockfd);
			return PARSE_REGISTER_FAILED;
		}
	} 
	else {
		if( pvt->sockfd == sipsock ) {
			cw_copy_string(p->transport, "UDP", sizeof(p->transport));
		} else {
			if (pvt->ssl)
				cw_copy_string(p->transport, "TLS", sizeof(p->transport));
			else if (pvt->sockfd != sipsock)
				cw_copy_string(p->transport, "TCP", sizeof(p->transport));
		}
	}
#endif

	if (c && cw_strlen_zero(p->username))
		cw_copy_string(p->username, c, sizeof(p->username));
#ifdef SIP_TCP_SUPPORT
 	else
		cw_copy_string(p->username, p->name, sizeof(p->username));
#endif		
    ...
    if (!peer  &&  autocreatepeer)
    {
        ...
        if (peer)
        {
            ...
            case PARSE_REGISTER_UPDATE:
                /* Say OK and ask subsystem to retransmit msg counter */
                transmit_response_with_date(p, "200 OK", req);
                manager_event(EVENT_FLAG_SYSTEM, "PeerStatus", "Peer: SIP/%s\r\nPeerStatus: Registered\r\nPeerType: SIP\r\nPeerIP: %s\r\nPeerPort: %d\r\nPeerExpire: %d\r\n", peer->name,  cw_inet_ntoa(iabuf, sizeof(iabuf), peer->addr.sin_addr), ntohs(peer->addr.sin_port), expiry);
                peer->lastmsgssent = -1;
                res = 0;
                break;
            }
...
static int get_destination(struct sip_pvt *p, struct sip_request *oreq)
{
    ...
    {
		if ( strncasecmp(from, "sip:", 4) == 0 ) {
			from += 4;
		} else if ( strncasecmp(from, "sips:", 5) == 0 ) {
			from += 5;
		} else {
 			cw_log(LOG_WARNING, "Huh?  Not a SIP header (%s)?\n", from);
 			return -1;
...
static int get_refer_info(struct sip_pvt *sip_pvt, struct sip_request *outgoing_req, char **transfercontext)
{
   ...
   if ( strncasecmp(refer_to, "sip:", 4) == 0) {
		refer_to += 4;
	} else if ( strncasecmp(refer_to, "sips:", 5) == 0) {
		refer_to += 5;
	} else {
		cw_log(LOG_ERROR, "Refer-to: Huh?  Not a SIP header (%s)?\n", refer_to);
 		return -1;
 	}	

	if ( strncasecmp(referred_by, "sip:", 4) == 0 ) {
		referred_by += 4;
	} else if ( strncasecmp(referred_by, "sips:", 5) == 0 ) {
		referred_by += 5;
	} else {
...
static int get_also_info(struct sip_pvt *p, struct sip_request *oreq)
{
    ...
    c = get_in_brackets(tmp);
	 ...
	 if ( strncmp(c, "sip:", 4) == 0 ) {
		c += 4;
	 } else 	if ( strncmp(c, "sips:", 5) == 0 ) {
		c += 5;
	 } else {
 		cw_log(LOG_WARNING, "Huh?  Not a SIP header (%s)?\n", c);
 		return -1;
...
static int check_via(struct sip_pvt *p, struct sip_request *req)
{
    ...
    if (c)
    {
        *c = '\0';
		...
		{
			cw_copy_string( transport, "UDP", sizeof(transport) );
		}else if (!strcasecmp(via, "SIP/2.0/TCP"))
		{
			cw_copy_string( transport, "TCP", sizeof(transport) );
		}else if (!strcasecmp(via, "SIP/2.0/TLS")) 
		{
			cw_copy_string( transport, "TLS", sizeof(transport) );
		}else {
#else
        if (strcasecmp(via, "SIP/2.0/UDP")){
#endif
...
#ifdef SIP_TCP_SUPPORT
       cw_copy_string( p->transport, transport, sizeof(p->transport) );
#endif /* SIP_TCP_SUPPORT */
...
#ifdef SIP_TCP_SUPPORT
   	   	cw_verbose("Sending to %s: %d; transport %s (%s)\n", cw_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr), ntohs(p->sa.sin_port), p->transport, c);
#else
    ...
    *t = '\0';
    ...
    if (cw_strlen_zero(p->exten))
    {
        ...
        if (strncasecmp(t, "sip:", 4) == 0)        
            t+= 4;
        else if (strncasecmp(t, "sips:", 5) == 0)
			   t+= 5;
...
static int sip_show_settings(int fd, int argc, char *argv[])
{
    ...
    cw_cli(fd, "  OSP Support:            No\n");		// Removed OSP support completely. 080528. mm_202.
...
static int sip_notify(int fd, int argc, char *argv[])
{
    ...
    if (!strcasecmp(argv[3], "like")) {
	if (argc <= 4 || regcomp(&regexbuf, argv[4], REG_EXTENDED | REG_NOSUB))
	    return RESULT_SHOWUSAGE;
	havepattern = 1;
    }
    ...
    CWOBJ_CONTAINER_TRAVERSE(&peerl, 1, do {
        ...
        if (havepattern && regexec(&regexbuf, iterator->name, 0, NULL, 0)) {
            ...
            found = 0;
            for(i = 3; i < argc; i++) {
		if (!strcasecmp(argv[i], iterator->name)) {
		  found = 1;
		}
	    }
	    if (found == 0) { 
        ...
        if (create_addr(p, iterator->name))
        {
...
"Usage: sip notify <type> [<peer>...|like <pattern>]\n"
...
static char *func_sipbuilddial_read(struct cw_channel *chan, int argc, char **argv, char *buf, size_t len)
{
    regex_t regexbuf;
    char data[1024];
    if (regcomp(&regexbuf, argv[0], REG_EXTENDED | REG_NOSUB)) {
       // Print Error 
       return "";
    }
    CWOBJ_CONTAINER_TRAVERSE(&peerl, 1, do {
       CWOBJ_RDLOCK(iterator);
       if (regexec(&regexbuf, iterator->name, 0, NULL, 0))
       {
            CWOBJ_UNLOCK(iterator);
            continue;
        }
       cw_copy_string(data, buf, sizeof(data));

       if (!cw_strlen_zero(data)) {
           snprintf(buf,len,"%s&SIP/%s",data,iterator->name);
       } else {
           snprintf(buf,len,"SIP/%s",iterator->name);
       }
...
static void parse_moved_contact(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (cw_test_flag(p, SIP_PROMISCREDIR)) {
		if ( strncasecmp(s, "sip:", 4) == 0 ) {
			s += 4;
		} else if ( strncasecmp(s, "sips:", 5) == 0 ) {
			s += 5;
		} 
		e = strchr(s, '/');
		if (e)
...
static int handle_request_invite(struct sip_pvt *p, struct sip_request *req, int debug, int ignore, int seqno, struct sockaddr_in *sin, int *recount, char *e)
{
    ...
    if (cw_test_flag(p, SIP_OUTGOING) && p->owner && (p->owner->_state != CW_STATE_UP))
    {
        ...
        if (debug) cw_verbose( "482 Loop Detected\n" );
...
#ifdef SIP_TCP_SUPPORT
static int check_content_length(char *data, int readbytes, int max)
{
	int f = 0, blen = 0, msglen = 0;
	char buf[SIP_MAX_PACKET];
	char *c = NULL, *header = NULL, *p;
			...
			*c = 0;
			...
			if (!strncmp(header, "Content-Length", strlen("Content-Length"))) {
				...
				if (p) {
					...
					cw_log(LOG_ERROR, "No colol in Content-Length header\n");
					return -1;
				}
			}
			if (cw_strlen_zero(header)) {
				/* Line by itself means we're now in content */
				c++;
				break;
			}
			if (f >= SIP_MAX_HEADERS - 1) {
				cw_log(LOG_WARNING, "Too many SIP headers...\n");
			} else
				f++;
			header = c + 1;
		} else if (*c == '\r') {
	...
	msglen = (int)(c - buf) + blen;

	/* cw_verbose("blen %d, msglen %d, readbytes %d\n", blen, msglen, readbytes); */
	if ((readbytes >= msglen) || (readbytes >= max))
		return msglen;
	else
		return 0;
}
...
static int sipsock_read(int *id, int fd, short events, void *ignore)
{
    ...
    memset(&req, 0, sizeof(req));
#ifdef SIP_TCP_SUPPORT
        if (fd != sipsock) {    /* It is TCP socket */
               if (getpeername(fd, (struct sockaddr *)&sin, &len) < 0)
                       cw_log(LOG_WARNING, "TCP getpeername error: %s\n", strerror(errno));

               /* ToDo: TLS read needs check the whole SIP message is read or not
                  by looking at double CRLF and Contect-Length header */

               if (ssl) {      /* This must be TLS connection socket */
                       do {
                               res = SSL_read(ssl, req.data+n, sizeof(req.data) - 1);
                               switch(SSL_get_error(ssl, res)) {
                                       case SSL_ERROR_NONE:
                                               n += res;
                                               break;
                                       case SSL_ERROR_ZERO_RETURN:             /* The peer closed the connection */
                                               cw_log(LOG_NOTICE, "The peer closed TLS Connection\n");
                                               res = -1;
                                               break;
                                       case SSL_ERROR_SYSCALL:
                                               cw_log(LOG_ERROR, "TLS SSL_ERROR_SYSCALL\n");
                                               res = -1;
                                               break;
                                       default:
                                               cw_log(LOG_ERROR, "TLS read error %d, %s\n", SSL_get_error(ssl, res), strerror(errno));
                                               res = -1;
                                               break;
                               }
                               if (res >= 0)
                                       msglen = check_content_length(req.data, n, (sizeof(req.data) - 1));
                       } while (!msglen && (res >= 0));
               } else {                                /* This must be TCP connection socket */
                       do {
                               res = read(fd, req.data+n, sizeof(req.data) - 1);
                               if (res > 0) {
                                       n += res;
                               } else if (res == 0) {   /* The client closed the TCP connection? */
                                       cw_log(LOG_NOTICE, "Remote closed TCP connection : fd %d\n", fd);
                                       res = -1;
                               }
                               else if (res < 0) {
                                       cw_log(LOG_ERROR, "TCP read error: %s\n", strerror(errno));
                               }
                               if (res >= 0)
                                       msglen = check_content_length(req.data, n, (sizeof(req.data) - 1));
                       } while (!msglen && (res >= 0));
               }
               /* clean up sockfd and ssl if a peer closed the TCP connection */
               if (res < 0) {
                       cleanup_tcp_connection(fd, ssl);
                       return 0;         /* Remove it */
               }
               res = msglen;
        } else
    ...
    {
#ifdef SIP_TCP_SUPPORT
        cw_verbose("\n<-- Sip read from %s:%d:%s \n%s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port), fd == sipsock ? "UDP" : (ssl ? "TLS" : "TCP"), req.data);
#else    
...
#ifdef SIP_TCP_SUPPORT
		p->sockfd = fd;         /* Save socket fd to send a response */
		if (fd != sipsock) {
			if (ssl) {
				p->ssl = ssl;   /* Only TLS will have non-null ssl */
				cw_copy_string(p->transport, "TLS", sizeof(p->transport));
			} else
				cw_copy_string(p->transport, "TCP", sizeof(p->transport));
		}
...
#ifdef SIP_TCP_SUPPORT
static int siptcp_accept(int *id, int fd, short events, void *ignore)
{
	int tcpconnfd = -1;
	...
	sa_len = sizeof(cliaddr);
	if ((tcpconnfd = accept(siptcpsock, (struct sockaddr *)&cliaddr, &sa_len)) < 0) {
		cw_log(LOG_WARNING, "Failed to accept SIP TCP connection from TCP listening sock %d : %s\n",
			siptcpsock, strerror(errno));
		return 1;
	}

	if( sip_debug_test_addr(&cliaddr) )
		cw_verbose(VERBOSE_PREFIX_2 "Accepted TCP connection fd %d from %s:%d\n",
			tcpconnfd, cw_inet_ntoa(iabuf, sizeof(iabuf), cliaddr.sin_addr), ntohs(cliaddr.sin_port));
	cw_io_add(io, tcpconnfd, sipsock_read, CW_IO_IN, NULL);
	return 1;
}

static int siptls_accept(int *id, int fd, short events, void *ignore)
{
	int ret, tlsconnfd = -1;
	struct sockaddr_in cliaddr;
	socklen_t sa_len;
	char iabuf[INET_ADDRSTRLEN];
	BIO *bio;
//	BIO *bio_err=BIO_new_fp(stderr, BIO_NOCLOSE);	// debug only
	SSL *ssl;
		
	sa_len = sizeof(cliaddr);
	if ((tlsconnfd = accept(siptlssock, (struct sockaddr *)&cliaddr, &sa_len)) < 0) {
		cw_log(LOG_WARNING, "Failed to accept SIP TLS connection from TLS listening sock %d : %s\n",
			siptlssock, strerror(errno));
		return 1;
	}

	/* Initiate TLS handshake */
	bio = BIO_new_socket(tlsconnfd, BIO_CLOSE);
	if (!(ssl = SSL_new(tlsctx))) {
		cw_log(LOG_ERROR, "SSL_new error : %s\n", ERR_reason_error_string(ERR_get_error()));
//		ERR_print_errors(bio_err);
	...
	if((ret = SSL_accept(ssl) <= 0)) {
		cw_log(LOG_ERROR, "SSL accept error : %s\n", ERR_reason_error_string(ERR_get_error()));
//		ERR_print_errors(bio_err);
		return 3;
	}
	
	/* ToDo : Client authentication code */
	
	if( sip_debug_test_addr(&cliaddr) )
		cw_verbose(VERBOSE_PREFIX_2 "Accepted TLS connection fd %d from %s:%d\n",
			tlsconnfd, cw_inet_ntoa(iabuf, sizeof(iabuf), cliaddr.sin_addr), ntohs(cliaddr.sin_port));

	if (!cw_io_add(io, tlsconnfd, sipsock_read, CW_IO_IN, ssl))
		cw_log(LOG_ERROR, "ist_io_add failed\n");
...
#ifdef SIP_TCP_SUPPORT
        if (siptcpsock > -1)
                siptcpsock_read_id = cw_io_add(io, siptcpsock, siptcp_accept, CW_IO_IN, NULL);
        if (siptlssock > -1)
                siptlssock_read_id = cw_io_add(io, siptlssock, siptls_accept, CW_IO_IN, NULL);
#endif
...
#ifdef SIP_TCP_SUPPORT
			if (siptcpsock > -1)
			         siptcpsock_read_id = cw_io_change(io, siptcpsock_read_id, siptcpsock, NULL, 0, NULL);
			if (siptlssock > -1)
			         siptlssock_read_id = cw_io_change(io, siptlssock_read_id, siptlssock, NULL, 0, NULL);
#endif
        }
...
static int sip_poke_peer(struct sip_peer *peer)
{
	...
	p->ssl = peer->ssl;
	cw_copy_string(p->transport, peer->transport, sizeof(p->transport));
	if (sipdebug)
		cw_log(LOG_NOTICE, "Poking %s: transport %s, sockfd %d \n", peer->username, p->transport, p->sockfd);
#endif
...
static struct sip_user *build_user(const char *name, struct cw_variable *v, int realtime, struct cw_config *cfg)
{
    ...
    int done = 0;
    struct cw_variable *v_start;
    struct cw_variable *v_tmp;
    v_start = v;
    v_tmp = v;
	...
	if (v_tmp && cfg != NULL) {
	    while (v_tmp) {
                if(!strcasecmp(v_tmp->name, "include")) {
		    v = cw_variable_browse(cfg, v_tmp->value);
		    v_tmp = v_tmp->next;
		    break;
		}
		v_tmp = v_tmp->next;
...
/*	peer->addr.sin_family = AF_INET; */
	...
	if(!peer->ssl)
		peer->ssl = NULL;
	cw_copy_string(peer->transport, DEFAULT_TRANSPORT, sizeof(peer->transport));
#endif
    cw_copy_flags(peer, &global_flags, SIP_FLAGS_TO_COPY);
    peer->capability = global_capability;
    peer->rtptimeout = global_rtptimeout;
		...
		cw_copy_string(peer->name, v->value, sizeof(peer->name));
            ...
            else if (!strcasecmp(v->name, "host") || !strcasecmp(v->name, "outboundproxy"))
            {
                ...
                else
                {
                    ...
                    {
#ifdef SIP_TCP_SUPPORT
					       if ( cw_get_ip_or_srv(&peer->addr, v->value, "_sip._udp") < 0 &&
						      cw_get_ip_or_srv(&peer->addr, v->value, "_sip._tcp") < 0 &&
						      cw_get_ip_or_srv(&peer->addr, v->value, "_sip._tls") < 0 &&
						      cw_get_ip_or_srv(&peer->addr, v->value, "_sipinternal._udp") < 0 &&
						      cw_get_ip_or_srv(&peer->addr, v->value, "_sipinternal._tcp") < 0 &&
						      cw_get_ip_or_srv(&peer->addr, v->value, "_sipinternal._tls") < 0 ) {
#else
                        if (cw_get_ip_or_srv(&peer->addr, v->value, (srvlookup)  ?  "_sip._udp"  :  NULL))
                        {
#endif /* SIP_TCP_SUPPORT */
  						          cw_log(LOG_WARNING, "Unable to locate outboundproxy for peer'%s'\n", v->value);
                            CWOBJ_UNREF(peer, sip_destroy_peer);
                            return NULL;
                        }
                    }
            ...
            else if (!strcasecmp(v->name, "qualify"))
            {
                ...
                else if (sscanf(v->value, "%d", &peer->maxms) != 1)
                {
                    ...
                    peer->maxms = 0;
			         ...
			         if(!strncasecmp(v->value, "udp", 3)) {
				      cw_copy_string(peer->transport, "UDP", sizeof(peer->transport));
			   } else if(!strncasecmp(v->value, "tcp", 3)) {
				      cw_copy_string(peer->transport, "TCP", sizeof(peer->transport));
			   } else if(!strncasecmp(v->value, "tls", 3)) {
				      cw_copy_string(peer->transport, "TLS", sizeof(peer->transport));
			   } else {
				      cw_log(LOG_WARNING, "Unsupported transport '%s' should be 'UDP', 'TCP', or 'TLS' at line %d of sip.conf\n", v->value, v->lineno);
			   }
#endif /* SIP_TCP_SUPPORT */
            }
            /* else if (strcasecmp(v->name,"type"))
...
static int reload_config(void)
{
...
#ifdef SIP_TCP_SUPPORT
 	 struct sockaddr_in old_tlsbindaddr = tlsbindaddr;
#endif
    ...
    global_allowguest = 1;
#ifdef SIP_TCP_SUPPORT
    snprintf(trustcerts_file, sizeof(trustcerts_file),"%s/trustcerts.pem", cw_config_CW_KEY_DIR);
    snprintf(servercert_file, sizeof(servercert_file),"%s/servercert.pem", cw_config_CW_KEY_DIR);
    snprintf(serverkey_file, sizeof(serverkey_file),"%s/serverkey.pem", cw_config_CW_KEY_DIR);
    snprintf(dh512param_file, sizeof(dh512param_file),"%s/dh512.pem", cw_config_CW_KEY_DIR);
    snprintf(dh1024param_file, sizeof(dh1024param_file),"%s/dh1024.pem", cw_config_CW_KEY_DIR);
#endif
    ...
    while (v)
    {
        ...
        {
#ifdef SIP_TCP_SUPPORT
			if ( cw_get_ip_or_srv(&outboundproxyip, v->value, "_sip._udp") < 0 &&
				cw_get_ip_or_srv(&outboundproxyip, v->value, "_sip._tcp") < 0 &&
				cw_get_ip_or_srv(&outboundproxyip, v->value, "_sip._tls") < 0 &&
				cw_get_ip_or_srv(&outboundproxyip, v->value, "_sipinternal._udp") < 0 &&
				cw_get_ip_or_srv(&outboundproxyip, v->value, "_sipinternal._tcp") < 0 &&
				cw_get_ip_or_srv(&outboundproxyip, v->value, "_sipinternal._tls") < 0 )
#else
            if (cw_get_ip_or_srv(&outboundproxyip, v->value, (srvlookup)  ?  "_sip._udp"  :  NULL) < 0)
                cw_log(LOG_WARNING, "Unable to locate host '%s'\n", v->value);
#endif /* SIP_TCP_SUPPORT */
				cw_log(LOG_WARNING, "Unable to locate outboundproxy host '%s'\n", v->value);
        }
...
#ifdef SIP_TCP_SUPPORT
		else if (!strcasecmp(v->name, "tlsport")) {
			if (sscanf(v->value, "%d", &ourport) == 1) {
				tlsbindaddr.sin_port = htons(ourport);
			} else {
				cw_log(LOG_WARNING, "Invalid tlsport number '%s' at line %d of %s\n", v->value, v->lineno, config);
			}
		} else if (!strcasecmp(v->name, "trustcerts")) {
			cw_copy_string(trustcerts_file, v->value, sizeof(trustcerts_file));
		} else if (!strcasecmp(v->name, "servercert")) {
			cw_copy_string(servercert_file, v->value, sizeof(servercert_file));
		} else if (!strcasecmp(v->name, "serverkey")) {
			cw_copy_string(serverkey_file, v->value, sizeof(serverkey_file));
		} else if (!strcasecmp(v->name, "serverkeypassword")) {
			cw_copy_string(serverkey_password, v->value, sizeof(serverkey_password));
		} else if (!strcasecmp(v->name, "dh512param")) {
			cw_copy_string(dh512param_file, v->value, sizeof(dh512param_file));
		} else if (!strcasecmp(v->name, "dh1024param")) {
			cw_copy_string(dh1024param_file, v->value, sizeof(dh1024param_file));
		}
#endif		
         v = v->next;
    }
    ...
    while (cat)
    {
        if (strcasecmp(cat, "general") && strcasecmp(cat, "authentication"))
        {
            ...
            if (utype)
            {
                ...
                {
                    user = build_user(cat, cw_variable_browse(cfg, cat), 0, cfg);
                    if (user)
                    {
                        CWOBJ_CONTAINER_LINK(&userl,user);
                        CWOBJ_UNREF(user, sip_destroy_user);
                ...
                {
                    peer = build_peer(cat, cw_variable_browse(cfg, cat), 0, cfg);
                    if (peer)
                    {
                        CWOBJ_CONTAINER_LINK(&peerl,peer);
                        CWOBJ_UNREF(peer, sip_destroy_peer);
...
#ifdef SIP_TCP_SUPPORT
	if ((siptcpsock > -1) && (memcmp(&old_tlsbindaddr, &bindaddr, sizeof(struct sockaddr_in)))) {
		close(siptcpsock);
		siptcpsock = -1;
	}
	...
	if (siptcpsock < 0) {
		...
		if (siptcpsock < 0) {
			cw_log(LOG_WARNING, "Unable to create SIP TCP socket: %s\n", strerror(errno));
		} else {
			/* Allow SIP clients on the same host to access us: */
			const int reuseFlag = 1;
			...
			if (bind(siptcpsock, (struct sockaddr *)&bindaddr, sizeof(bindaddr)) < 0) {
				cw_log(LOG_WARNING, "Failed to bind SIP TCP socket to %s:%d: %s\n",
					cw_inet_ntoa(iabuf, sizeof(iabuf), bindaddr.sin_addr), ntohs(bindaddr.sin_port),
					strerror(errno));
				close(siptcpsock);
				siptcpsock = -1;
				...
				if (option_verbose > 1) { 
					cw_verbose(VERBOSE_PREFIX_2 "SIP TCP Listening on %s:%d\n", 
					cw_inet_ntoa(iabuf, sizeof(iabuf), bindaddr.sin_addr), ntohs(bindaddr.sin_port));
					cw_verbose(VERBOSE_PREFIX_2 "Using TOS bits %d\n", tos);
				}
				if (setsockopt(siptcpsock, IPPROTO_IP, IP_TOS, &tos, sizeof(tos)))
					cw_log(LOG_WARNING, "Unable to set TOS to %d in SIP TCP\n", tos);
				if (listen(siptcpsock, 30) < 0) {
					cw_log(LOG_WARNING, "Failed to listen on SIP TCP\n");
				}
			}
		}
	}

	/* Open a TLS listening socket */
	memcpy(&tlsbindaddr.sin_addr, &bindaddr.sin_addr, sizeof(tlsbindaddr.sin_addr));
	if (!ntohs(tlsbindaddr.sin_port))
		tlsbindaddr.sin_port = ntohs(DEFAULT_SIP_TLS_PORT);
	tlsbindaddr.sin_family = AF_INET;
	if ((siptlssock > -1) && (memcmp(&old_tlsbindaddr, &tlsbindaddr, sizeof(struct sockaddr_in)))) {
		close(siptlssock);
		siptlssock = -1;
	}
	if (siptlssock < 0) {
		siptlssock = socket(AF_INET, SOCK_STREAM, 0);
		if (siptlssock < 0) {
			cw_log(LOG_WARNING, "Unable to create SIP TLS socket: %s\n", strerror(errno));
		} else {
			/* Allow SIP clients on the same host to access us: */
			const int reuseFlag = 1;
			setsockopt(siptlssock, SOL_SOCKET, SO_REUSEADDR, (
				   const char *)&reuseFlag,
				   sizeof reuseFlag);

			if (bind(siptlssock, (struct sockaddr *)&tlsbindaddr, sizeof(tlsbindaddr)) < 0) {
				cw_log(LOG_WARNING, "Failed to bind SIP TLS socket to %s:%d: %s\n",
					cw_inet_ntoa(iabuf, sizeof(iabuf), tlsbindaddr.sin_addr), ntohs(tlsbindaddr.sin_port),
					strerror(errno));
				close(siptlssock);
				siptlssock = -1;
			} else {
				if (option_verbose > 1) { 
					cw_verbose(VERBOSE_PREFIX_2 "SIP TLS Listening on %s:%d\n", 
					cw_inet_ntoa(iabuf, sizeof(iabuf), tlsbindaddr.sin_addr), ntohs(tlsbindaddr.sin_port));
					cw_verbose(VERBOSE_PREFIX_2 "Using TOS bits %d\n", tos);
				}
				if (setsockopt(siptlssock, IPPROTO_IP, IP_TOS, &tos, sizeof(tos)))
					cw_log(LOG_WARNING, "Unable to set TOS to %d in SIP TLS\n", tos);
				if (listen(siptlssock, 30) < 0) {
					cw_log(LOG_WARNING, "Failed to listen on SIP TLS\n");
				}
			}
		}
	}
 ...
 */
static int sip_sendtext2(struct cw_channel *chan, const char *text, const char *disp) {
	struct sip_request req;
	struct sip_pvt *p = chan->tech_pvt;
	//int res = 0;
	if (!p) return -1;
...
DH *read_dhparams(char *dhfile)
{
	...
	if (!bio)
		cw_log(LOG_ERROR, "Error opening file %s", dhfile);
	dh = PEM_read_bio_DHparams(bio, NULL, NULL, NULL);
	if (!dh)
		cw_log(LOG_ERROR, "Error reading DH parameters from %s", dhfile);
	BIO_free(bio);
	return dh;
}
...
DH *tmp_dh_callback(SSL *ssl, int is_export, int keylength)
{
	...
	{
		case 512:
			ret = read_dhparams(dh512param_file);
			break;
		case 1024:
		default:
			ret = read_dhparams(dh1024param_file);
			break;
	}
	return ret;
}
...
{
	if (num < (strlen(serverkey_password)+1)) {
		cw_log(LOG_ERROR, "password buf len %d is too small\n", num);
		return 0;
	}
	strcpy(buf, serverkey_password);
	return (strlen(serverkey_password));
}
...
int verify_callback(int ok, X509_STORE_CTX *store)
{
    ...
    if (!ok)
    {
        ...
        cw_log(LOG_ERROR, "-Error with certificate at depth: %i\n", depth);
        X509_NAME_oneline(X509_get_issuer_name(cert), data, 256);
        cw_log(LOG_ERROR, "  issuer   = %s\n", data);
        X509_NAME_oneline(X509_get_subject_name(cert), data, 256);
        cw_log(LOG_ERROR, "  subject  = %s\n", data);
        cw_log(LOG_ERROR, "  err %i:%s\n", err, X509_verify_cert_error_string(err));
    }
 
    return ok;
}

/* initailize OpenSSL library */
SSL_CTX *init_OpenSSL(void)
{
	SSL_CTX *ctx = NULL;
	
	if (!SSL_library_init()) {
		cw_log(LOG_ERROR, "SSL_library_init failed\n");
		return NULL;
	}
	SSL_load_error_strings();
	RAND_load_file(DEFAULT_ENTROPY, 1024);

	ctx = SSL_CTX_new(TLSv1_method());
	
	if (SSL_CTX_load_verify_locations(ctx, trustcerts_file, NULL) != 1)
		cw_log(LOG_ERROR, "Error loading a trust certs\n");

	SSL_CTX_set_default_passwd_cb(ctx, password_callback);
	
	if (SSL_CTX_set_default_verify_paths(ctx) != 1)
		cw_log(LOG_ERROR, "Error to set_default_verify_path\n");
	if (SSL_CTX_use_certificate_chain_file(ctx, servercert_file) != 1)
		cw_log(LOG_ERROR, "Error loading certificate from file\n");
	if (SSL_CTX_use_PrivateKey_file(ctx, serverkey_file, SSL_FILETYPE_PEM) != 1)
		cw_log(LOG_ERROR, "Error loading private key from file\n");
...
/*	SSL_CTX_set_verify_depth(ctx, 4); */
	...
	if (SSL_CTX_set_cipher_list(ctx, CIPHER_LIST) != 1)
		cw_log(LOG_ERROR, "Error setting cipher list (no valid ciphers)\n");
...
int load_module(void)
{
    ...
    sipbuilddial_function = cw_register_function(sipbuilddial_func_name, func_sipbuilddial_read, NULL, sipbuilddial_func_synopsis, sipbuilddial_func_syntax, sipbuilddial_func_desc);
...
int unload_module(void)
{
    ...
    cw_unregister_function(sipbuilddial_function);
...
#ifdef SIP_TCP_SUPPORT
	CWOBJ_CONTAINER_TRAVERSE(&peerl, 1, {
		if (iterator->ssl) {
			SSL_clear(iterator->ssl);
			SSL_free(iterator->ssl);
		}
	...
	close(siptcpsock);
	close(siptlssock);
	SSL_CTX_free(tlsctx);   /* destroy TLS CTX */
#endif	    
    return res;
}
