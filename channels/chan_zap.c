...
CALLWEAVER_FILE_VERSION("$HeadURL: https://svn.callweaver.org/callweaver/branches/rel/1.2/channels/chan_zap.c $", "$Revision: 5231 $")
...
static struct cw_channel *zt_request(const char *type, int format, void *data, int *cause)
{
	...
	if (!deprecated && !strcmp(type, zap_tech.type)) {
		deprecated = 1;
		cw_log(LOG_WARNING, "Channel type Zap/... is deprecated. Use DAHDI/... instead.\n");
	}
...
static struct cw_cli_entry zap_cli[] = {
	{ { "dahdi", "show", "cadences", NULL }, handle_zap_show_cadences,
	  "List cadences", zap_show_cadences_help },
	{ {"dahdi", "show", "channels", NULL}, zap_show_channels,
	  "Show active DAHDI channels", show_channels_usage },
	{ {"dahdi", "show", "channel", NULL}, zap_show_channel,
	  "Show information on a channel", show_channel_usage },
	{ {"dahdi", "destroy", "channel", NULL}, zap_destroy_channel,
	  "Destroy a channel", destroy_channel_usage },
	{ {"dahdi", "show", "status", NULL}, zap_show_status,
	  "Show all DAHDI cards status", zap_show_status_usage },
...
static int action_zapdndon(struct mansession *s, struct message *m)
{
	...
	if (!deprecated && (hdr == astman_get_header(m, "Action")) && !strncasecmp(hdr, "Zap", 3)) {
		deprecated = 1;
		cw_log(LOG_WARNING, "Manager action %s is deprecated. Use DAHDI%s instead\n", hdr, hdr+3);
	}
...
static int action_dahdidisableec(struct cw_channel *chan, int argc, char **argv)
{
    if (chan==NULL) {
	...
	cw_log(LOG_WARNING, "channel is not DAHDI or Zap\n");
        return 0;
    }
    struct zt_pvt *p = chan->tech_pvt;
    if (p==NULL) {
...
static int action_zapdisableec(struct cw_channel *chan, int argc, char **argv)
{
	...
	if (!deprecated) {
		deprecated = 1;
		cw_log(LOG_WARNING, "%s is deprecated. Use %s instead\n", zapdisableec_name, dahdidisableec_name);
	}

	return action_dahdidisableec(chan, argc, argv);
}
...
static int __unload_module(void)
{
	...
	cw_unregister_application(dahdidisableec_app);
	...
	cw_unregister_application(zapdisableec_app);
...
static int setup_zap(int reload)
{
	...
	if (!(cfg = cw_config_load(config_dahdi))) {
		if ((cfg = cw_config_load(config_zapata)))
			cw_log(LOG_WARNING, "zapata.conf is deprecated. Rename it to chan_dahdi.conf\n");
	}

	/* We *must* have a config file otherwise stop immediately */
	if (!cfg) {
		cw_log(LOG_ERROR, "Unable to load config %s or %s\n", config_dahdi, config_zapata);
		return -1;
	}
...
int load_module(void)
{
	...
	dahdidisableec_app = cw_register_application(dahdidisableec_name, action_dahdidisableec, dahdidisableec_synopsis, dahdidisableec_syntax, dahdidisableec_description);
	...
	zapdisableec_app = cw_register_application(zapdisableec_name, action_zapdisableec, dahdidisableec_synopsis, zapdisableec_syntax, dahdidisableec_description);
